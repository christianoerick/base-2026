...
class SeedContentCommand extends Command
{
	protected $signature = 'admin:seed
							{type : Tipo de conteúdo (site|portal|blog|tv)}
							{--count=20 : Quantidade de posts/páginas}
							{--domains=1 : Quantidade de domínios}
	...
	// Dados em português para geração
	...
	public function handle()
	{
		...
		if (!in_array($type, ['site', 'portal', 'blog', 'tv'])) {
			$this->error("❌ Tipo inválido! Use: site, portal, blog ou tv");
	...
	protected function createDomains($count)
	{
		$this->task('Criando domínios', function () use ($count, &$domains) {
			$domains = collect();
			$domainNames = ['principal', 'regional', 'corporativo', 'institucional', 'mobile'];

			for ($i = 0; $i < $count; $i++) {
				$domains->push(Domain::create([
					'name' => $domainNames[$i] ?? "dominio-{$i}",
					'url' => "https://{$domainNames[$i]}.exemplo.com.br",
					'is_active' => true
				]));
			}
